...
class AttackLog:
    @staticmethod
    def create(attack_type, endpoint, user_id, ip_address, user_agent, request_method, payload, severity, details=None):
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
            '''INSERT INTO attack_logs 
               (attack_type, endpoint, user_id, ip_address, user_agent, request_method, payload, severity, details)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (attack_type, endpoint, user_id, ip_address, user_agent, request_method, payload, severity, details)
        )
        conn.commit()
